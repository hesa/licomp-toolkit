...
test_snippets()
{
    ...
    test_licomp_tk "-u snippet verify -il MIT -ol MIT" "${EXTRACT_COMPAT}.summary.results.nr_valid" 1
    test_licomp_tk "-u snippet verify -il MIT -ol MIT2" "${EXTRACT_COMPAT}.summary.results.nr_valid" 0
    test_licomp_tk "-u snippet verify -il BSD-3-Clause -ol LGPL-2.1-or-later" "${EXTRACT_COMPAT}.summary.results.nr_valid" 1
    test_licomp_tk "-u snippet verify -il LGPL-2.1-or-later -ol BSD-3-Clause" "${EXTRACT_COMPAT}.summary.results.nr_valid" 1
    test_licomp_tk "-u snippet verify -il BSD-3-Clause -ol LGPL-2.1-or-later" "${EXTRACT_COMPAT}.summary.results.yes.count" 1
    test_licomp_tk "-u snippet verify -il LGPL-2.1-or-later -ol BSD-3-Clause" "${EXTRACT_COMPAT}.summary.results.yes.count" null
}
